...
|---|---|
| [`autocare-platform`](https://github.com/SrikanthITSolutions/autocare-platform) | Java 17 / Spring Boot application source, Maven build, unit tests, `Dockerfile`, application config |
| [`autocare-infrastructure`](https://github.com/SrikanthITSolutions/autocare-infrastructure) | Terraform: VPC, subnets, NAT, EKS, managed node groups, RDS, ECR, IAM, Pod Identity/IRSA, Secrets Manager, CloudWatch |
...
AWS Secrets Manager (autocare/<env>/database)
   ...
   → AutoCare Pod env vars (secretKeyRef: DB_HOST, DB_PORT, DB_NAME, DB_USERNAME, DB_PASSWORD, REMEMBER_ME_KEY)
...
- An AWS Secrets Manager secret named `autocare/<env>/database` containing `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USERNAME`, `DB_PASSWORD`, `REMEMBER_ME_KEY`
...
```json
{
  "DB_HOST": "autocare-dev.xxxxxxxxxxxx.us-east-1.rds.amazonaws.com",
  "DB_PORT": "3306",
  "DB_NAME": "autocare",
  "DB_USERNAME": "autocare_admin",
  "DB_PASSWORD": "REPLACE_ME_IN_SECRETS_MANAGER",
  "REMEMBER_ME_KEY": "REPLACE_ME_IN_SECRETS_MANAGER"
}
```

Both `DB_PASSWORD` and `REMEMBER_ME_KEY` are generated by Terraform
(`random_password`) in `autocare-infrastructure` — this repo and its
operators never see or set them.
...
helm template autocare ./helm/autocare \
  ...
  --set image.repository=<account-id>.dkr.ecr.us-east-1.amazonaws.com/autocare \
  --set image.tag=<tag>
...
```bash
aws eks update-kubeconfig --name autocare-dev-eks --region us-east-1
kubectl get nodes
```
...
```bash
./scripts/deploy.sh dev <image-tag> <ecr-repository> us-east-1 autocare-dev-eks autocare <irsa-role-arn>
```
...
```bash
./scripts/deploy.sh prod <image-tag> <ecr-repository> us-east-1 autocare-prod-eks autocare <irsa-role-arn>
```
...
- **Pod `CrashLoopBackOff` on DB connection** → verify the synced Secret (`kubectl get secret <syncSecretName> -n autocare -o yaml`) actually contains `DB_HOST/PORT/NAME/USERNAME/PASSWORD/REMEMBER_ME_KEY`, and that the pod's IAM role can read the AWS secret.
